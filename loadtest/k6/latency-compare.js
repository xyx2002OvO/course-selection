...
// Collapse ladder to 1000. Pairing rotates course on each student pass so
// 60000%20=0 does not pin every student to one course. Timeout 8s matches Dubbo.
const pieces = [
...
function studentForIteration(from, to, iteration) {
  const span = to - from + 1;
  return from + (iteration % span);
}

function courseForIteration(from, to, iteration) {
  const nStudents = env.studentTo - env.studentFrom + 1;
  const nCourses = to - from + 1;
  const pass = Math.floor(iteration / nStudents);
  return from + ((iteration + pass) % nCourses);
}
...
export default function (data) {
  ...
  const res = http.post(
    ...
    { headers, tags: { name: 'submit', ...tags }, timeout: '8s' },
  );
...
function taggedDuration(data, stageName) {
  for (const [name, metric] of Object.entries(data.metrics || {})) {
    ...
    return {
      p50: v.med != null ? Number(v.med) : (v['p(50)'] != null ? Number(v['p(50)']) : null),
...
export function handleSummary(data) {
  ...
  const stages = holds.map((p) => {
    ...
    return {
      ...
      p50_ms: latency.p50 == null ? null : Number(latency.p50.toFixed(1)),
  ...
  const report = {
    scenario: 'http-accept-latency',
    mode: __ENV.SYNC_BASELINE === 'true' ? 'sync' : 'async',
...
function textReport(report) {
  const lines = [
    ...
    '台阶 (仅 hold；受理=202/200；p50/p95 只统计成功 HTTP):',
  ];
  for (const row of report.stages) {
    lines.push(
      `  ${row.stage} target=${row.target_rps} realized=${row.realized_accept_qps} `
      + `accept=${pct(row.accept_rate)} p50=${row.p50_ms ?? 'n/a'}ms p95=${row.p95_ms ?? 'n/a'}ms `
